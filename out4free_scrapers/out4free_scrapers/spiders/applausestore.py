...
class ApplausestoreSpider(scrapy.Spider):
    name = "applausestore"
    allowed_domains = ["applausestore.com"]
    start_urls = ['https://www.applausestore.com/calendar']

    def parse(self, response):
        urls = response.xpath('//li[@class="showbook"]/a[contains(span,"Booking Now")]/@href').extract()
        for url in urls:
            full_url = response.urljoin(url)
            yield scrapy.Request(full_url, callback=self.individual_page)

    def individual_page(self, response):
        title = response.xpath('//div[@class="detailtext"]/h1/text()').extract_first()
        description = response.xpath('//div[@class="detailtext"]/p/text()').extract()[:-2]
        description = " ".join(description)
        price = 'free'
        date = response.xpath('//div[@class="tickdate"]/a/text()').extract_first()
        time = response.xpath('//div[@class="tickdate"]/span[@class="darkgrey2"]/text()').extract_first()
        start_datetime = date + time
        category_id = CatItem.django_model.objects.get(id=8)
        venue_name = response.xpath('//div[@class="studgreybox"]/a/text()').extract_first()
        ...
        elif "Elstree Film Studios" in venue_name:
            venue_id = VenueItem.django_model.objects.get(id=128)
        elif "Wapping" in venue_name:
            venue_id = VenueItem.django_model.objects.get(id=121)
        else:
            venue_id = VenueItem.django_model.objects.get(id=0)

        slug = slugify(title)
        image_urls = response.xpath('//div[@class="detailsimage"]/img/@src').extract_first()
